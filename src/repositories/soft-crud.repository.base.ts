...
export abstract class SoftMetaCrudRepository<T extends MetaEntity,
    ID,
    ...
    private async prepaidEntity (entity : DataObject<T>, getUserId = true, userId? : string) : Promise<DataObject<T>> {
        entity.createdOn = entity.createdOn || new Date();
        entity.updatedOn = new Date();

        entity.createdBy = entity.createdBy || getUserId ? await this.getUserId() : userId;
